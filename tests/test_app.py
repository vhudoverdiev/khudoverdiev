...
def test_allowed_host_with_port_is_accepted(client):
    ...
    assert response.status_code == 200
    assert b"css/styles.css?v=78" in response.data
...
def test_photo_client_page_renders_personal_redirect_buttons_without_storing_photos(client):
    ...
    assert "left: 50%;\n        width: calc(100% - 32px);\n        min-height: 48px;\n        margin: 0;" in css
    assert "justify-content: center;\n        transform: translateX(-50%);" in css
...
def test_inactive_or_missing_photo_client_shows_unavailable_state_without_links(client):
    ...
    assert "Страница фотографа".encode() in inactive.data
    assert "Персональная ссылка".encode() not in inactive.data
    assert b'class="client-header-badge"' in inactive.data
    assert b'href="https://ph.khudoverdiev.ru"' in inactive.data
    assert b"client-camera-body" in inactive.data
    assert b"client-camera-lens" in inactive.data
    assert "Напишите фотографу.".encode() in inactive.data
    assert "Напишите мне, и я проверю доступ.".encode() not in inactive.data
    css = Path("static/css/styles.css").read_text(encoding="utf-8")
    assert ".client-card-unavailable h1 {\n    position: relative;\n    z-index: 1;" in css
    assert ".client-card-unavailable .client-lead {\n    position: relative;\n    z-index: 1;" in css
